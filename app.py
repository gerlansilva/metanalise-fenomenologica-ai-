...
if st.session_state.analysis_done and st.session_state.result_data:
    ...
    if includes_systematic(render_mode):
        with st_tabs[tab_idx]:
            ...
            if not docs: st.warning("O mapeamento sistemático não foi retornado.")
            else:
                rows_long = []
                for doc in docs:
                    for ans in doc.get("respostas", []):
                        rows_long.append({"Documento": doc.get("documento"), "Pergunta": ans.get("pergunta"), "Resposta": ans.get("resposta"), "Evidência": ans.get("evidencia_textual"), "Página": ans.get("pagina")})
                df_long = pd.DataFrame(rows_long)
                
                st.download_button("Exportar CSV", df_long.to_csv(index=False).encode("utf-8"), "mapeamento.csv", "text/csv")
                perguntas = df_long["Pergunta"].dropna().unique().tolist()
                
                for pergunta in perguntas:
                    sub = df_long[df_long["Pergunta"] == pergunta].copy()
                    st.markdown(f'<div class="qa-shell" style="margin-top: 20px; margin-bottom: 20px;"><h4 style="margin:0; color:var(--accent2);">🔎 {pergunta}</h4></div>', unsafe_allow_html=True)
