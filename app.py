...
.chip{ border-radius: 12px; padding: 6px 14px; font-size: 13px; font-weight: 700; color: #FFF; background-color: var(--mint); margin-right: 5px;}
...
if not api_key:
    st.error("⚠️ Variável GEMINI_API_KEY não configurada.")
    st.stop()
...
class UnidadeSentido(BaseModel):
    ...
    documento: str
    pagina: int | None
    citacao_literal: str # IA deve formatar como "Trecho..." (p. X)
...
if run:
    ...
    def update_t():
        while not stop_timer:
            ...
            timer.markdown(f'<div style="background:#FFF; padding:15px; border-radius:15px; border:2px solid #17C3B2; text-align:center;">🧠 Analisando... {elapsed}s</div>', unsafe_allow_html=True)
            time.sleep(1)
    ...
    try:
        ...
        prompt = "Analise o corpus anexado.\n\n"
        if includes_phenom(mode):
            prompt += f"FENOMENOLOGIA: {phenom_q}\nExtraia US com página. FORMATE 'citacao_literal' como: \"Trecho...\" (p. X).\n"
        
        response = client.models.generate_content(
            model="gemini-2.5-flash",
            contents=parts + [prompt],
            config=types.GenerateContentConfig(
                system_instruction="Assistente de Pesquisa. Responda em JSON.",
...
if st.session_state.analysis_done and st.session_state.result_data:
    ...
    m = st.session_state.last_mode
    
    tabs_labels = []
    if includes_phenom(m): tabs_labels.extend(["☰ Unidades", "📄 Significados", "🏷️ Categorias"])
    if includes_thematic(m): tabs_labels.extend(["🧩 Códigos", "🗂️ Temas"])
    if includes_systematic(m): tabs_labels.append("🧭 Mapeamento")
    
    st_tabs = st.tabs(tabs_labels)
    idx = 0
    
    if includes_phenom(m):
        with st_tabs[idx]:
            for u in res.get("fenomenologico", {}).get("unidades_sentido", []):
        ...
        idx += 1
        with st_tabs[idx]:
            for s in res.get("fenomenologico", {}).get("unidades_significado", []):
                st.markdown(f'<div class="qa-shell"><span class="chip">{s["id_unidade"]}</span><div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-top:10px;"><div><small>TRECHO</small><div class="quote">{s["trecho_original"]}</div></div><div><small>SÍNTESE</small><div style="background:#f0f9f8; padding:15px; border-radius:15px; font-weight:700;">{s["sintese"]}</div></div></div></div>', unsafe_allow_html=True)
        idx += 1
        with st_tabs[idx]:
            cats = res.get("fenomenologico", {}).get("categorias", [])
            cols = st.columns(3)
            for i, c in enumerate(cats):
                with cols[i%3]:
                    st.markdown(f'<div class="qa-shell" style="height:100%; border-top:6px solid purple;"><h4>{c["nome"]}</h4><p style="font-size:14px; text-align:justify;">{c["descricao"]}</p><hr><small>RELACIONADAS:</small><br>{" ".join([f"<span class=\'chip\' style=\'background:purple; font-size:10px;\'>{us}</span>" for us in c.get("unidades_relacionadas", [])])}</div>', unsafe_allow_html=True)
        idx += 1

    if includes_systematic(m):
        with st_tabs[-1]:
            rows = []
            for doc in res.get("sistematico", {}).get("documentos", []):
